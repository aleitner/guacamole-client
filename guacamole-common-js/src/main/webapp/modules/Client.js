...
Guacamole.Client = function(tunnel) {
     ...
     * available for use, including comprehensive device information.
     *
    ...
    /**
     * Sends USB data to a specific endpoint on a USB device.
     *
     * @param {!string} deviceId
     *     The unique identifier for the USB device.
     * 
     ...
     */
    this.sendUSBData = function(deviceId, endpoint, data, length, type) {
        if (!isConnected())
            return;
        
        tunnel.sendMessage("usbdata", deviceId, endpoint, data, length, type);
    };
    ...
    /**
     * Fired when USB data is received for a specific device endpoint.
     * 
     * @event
     * @param {!string} deviceId
