    ...
    function usbService($injector) {
        ...
        service.requestDevice = function requestDevice(filters) {
            ...
            if (!service.isWebUSBSupported) {
                deferred.reject(new Error("WebUSB API is not available in this browser"));
         ...
         */
        service.connectDevice = function connectDevice(device, managedClient) {
            const deferred = $q.defer();
            ...
            if (service.isDeviceConnected(managedClient, device)) {
                const existingUSB = service.findManagedUSB(managedClient, device);
                console.log("Device already connected to client:", device);
            ...
            ManagedClient.connectUSBDevice(managedClient, device)
                .then(managedUSB => {
                    console.log("USB device connected successfully:", device);
         ...
         */
        service.disconnectDevice = function disconnectDevice(device, managedClient) {
            const deferred = $q.defer();
            ...
            if (!managedUSB) {
                console.log("Device not found in client's USB devices:", device);
                deferred.resolve();
                return deferred.promise;
            }
            
            console.log("Disconnecting USB device:", device);
            
            // Use ManagedClient's method to disconnect the device
            ManagedClient.disconnectUSBDevice(managedClient, managedUSB)
                .then(() => {
                    console.log("USB device disconnected successfully");
                    deferred.resolve();
                })
                .catch(error => {
